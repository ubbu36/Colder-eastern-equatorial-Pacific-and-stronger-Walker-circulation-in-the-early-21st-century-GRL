...
sst_main=xr.open_dataset('/Users/ullaheede_1/Downloads/HadISST_sst.nc')
sst_main=sst_main
sst_main= sst_main.assign_coords(longitude=((360 + (sst_main.longitude % 360)) % 360))
sst_main=sst_main.roll(longitude=180)
sst_main=sst_main.rename({'longitude': 'lon','latitude': 'lat'})


sst=sst_main.rolling(time=5*12, center=True).mean().sel(time=slice('1920-01-01','2016-12-30'))
sst=sst.groupby('time.year')-sst.groupby('time.year').mean('time')

sst_low=sst['sst'].sel(lat=slice(55,20),lon=slice(120,260))
...
cf1=axlist[0].contourf(eofs.lon, eofs.lat, eofs.sel(mode=1)*1000,levels, extend="both",
...
axlist[0].set_extent([-1790, 790, -90, 90], ccrs.PlateCarree(180))
axlist[0].coastlines()
#plt.colorbar()
#plt.clim(-0.7,0.7)
